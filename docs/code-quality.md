...
| Dependencies | `npm audit --omit=dev --audit-level=high`; Dependabot weekly with cooldowns — one grouped npm PR for all minor/patch bumps (single lockfile change, no rebase ping-pong under the strict up-to-date rule), `tools/media` monthly, actions incl. `.github/actions/*`; merge after `ci` is green (a Dependabot PR needs an owner approval before merge) | PR, main | yes |
